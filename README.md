...
* 在发布内部测试版的时候会改变四段数字标号的最后一位，并且主版本后缀恒为_Insider_Ayano_Aishi_Only（简化为_IAAO）
...
0.6.0_Pub:双击打开剧情文件的播放模式下，当用户退出UI后再重新进入UI时，程序会自动播放最初被双击打开的那个剧情，无论第一次进入UI后做了什么播放操作或者装载其他剧情文件。【问题不影响主要功能，不单开pub版本修复。】

0.6.0_Pub:在因未能匹配到正确版本而导致播放器无法播放时，再次出现首页后，首页上的按钮不起作用。【不影响主要功能，不单开pub版本修复。】

# Ver0.7.0_Pre4(Build85.0)_SPOL0.6.0
...
## Ver0.7.0_Pre3(Build80.1020)_SPOL0.6.0

完善了图形交互页面的部分功能

## Ver0.7.0_Pre2(Build76.0):
